...
export function formatAuditLog(toolName: string, args: unknown, state: TurnRiskState, sinks: SinkRisk[], assessment: RiskAssessment): string {
  ...
  return ['⚠ DSH Injection Guard', '', 'Possible indirect prompt injection detected.', '', 'Context sources:', ...state.sources.filter(s => s.trust !== 'TRUSTED').map(s => `  ${s.trust.toLowerCase()}: ${safeLogValue(s.label)}`), '', 'Injection signals:', ...state.injectionSignals.map(s => `  - ${safeLogValue(s.evidence)}`), '', 'Sensitive action:', `  tool: ${safeLogValue(toolName)}`, `  target: ${safeLogValue(target)}`, `  sink: ${sinks.map(s => s.type).join(', ') || 'none'}`, '', 'Reasons:', ...assessment.reasons.map(reason => `  - ${safeLogValue(reason)}`), '', 'Risk:', `  ${assessment.level} (${assessment.score}/100)`, '', 'Decision:', `  ${decision}`].join('\n')
}
